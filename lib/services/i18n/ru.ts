...
export const russianTranslations: Record<string, TranslationValue> = {
  ...
  "layout.restorePanel": "Восстановить панель Poe Zh Trade Tools Pro",
  "popup.description":
    "Poe Zh Trade Tools Pro добавляет более быструю навигацию и торговые инструменты на официальный сайт торговли Path of Exile.",
  ...
  "settings.sidebarDescription":
    "Выберите сторону экрана, где будет отображаться панель Poe Zh Trade Tools Pro.",
  "settings.left": "Слева",
  ...
  "about.description":
    "Poe Zh Trade Tools Pro — это дополнение для торговли Path of Exile, созданное для сохранения поисков, организации папок, отслеживания истории и удобной работы с повторяющимися торговыми сценариями прямо на официальном сайте.",
  ...
  "folder.addSubfolder": "Добавить подпапку",
  "folder.moveToFolder": "Переместить в папку",
  "folder.movedTrade": ({ title, folder }) => `"${title}" перемещено в ${folder}`,
  "folder.noOtherFolders": "Нет других папок",
