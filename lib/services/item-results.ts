...
const UI_LANGUAGE_TO_MAGEBLOOD_LOCALE: Record<string, MagebloodLegacyLocale> = {
  en: "en",
  es: "es",
  pt: "pt",
  ru: "ru",
...
// reverse maps, which the background stores.
let twNameReverse: Record<string, string> = {};
let cnNameReverse: Record<string, string> = {};
try {
  chrome.storage?.local?.get(["zhCore_reverse", "zhCn_reverse"], (r) => {
    const data = r as Record<string, Record<string, string> | undefined>;
    if (data?.zhCore_reverse) twNameReverse = data.zhCore_reverse;
    if (data?.zhCn_reverse) cnNameReverse = data.zhCn_reverse;
  });
  chrome.storage?.onChanged?.addListener((changes, area) => {
    if (area !== "local") return;
    if (changes.zhCore_reverse?.newValue)
      twNameReverse = changes.zhCore_reverse.newValue as Record<string, string>;
    if (changes.zhCn_reverse?.newValue)
      cnNameReverse = changes.zhCn_reverse.newValue as Record<string, string>;
  });
} catch {
  // chrome.storage unavailable in this context; poedb button will only work
  // when the header already exposes an English name.
}
...
export class ItemResultsService {
  ...
  private readonly handleDocumentClick = (event: MouseEvent) => {
    ...
    if (poedbButton && poedbLocale() && experimentalSettings.isPoedbVisible()) {
  ...
  async initialize() {
    ...
    this.unsubscribeSettings = settings.subscribe((value) => {
      ...
      const languageChanged =
        this.lastKnownLanguage !== null && this.lastKnownLanguage !== value.language;
      this.lastKnownLanguage = value.language;
      ...
      if (languageChanged) {
        // Language-dependent action buttons (Wiki = English only, PoeDB =
        // Chinese only) must be re-synced so stale ones are removed.
        this.resyncActionButtons();
      }
    });
    ...
    try {
      ...
      console.error("[Poe Zh Trade Tools Pro] Failed to fetch ratios from poe.ninja:", e);
    }
  ...
  private async handleLocationChange() {
    try {
      ...
      console.error("[Poe Zh Trade Tools Pro] Failed to refresh ratios after location change:", e);
    }
  ...
  private resyncActionButtons() {
    const rows = document.querySelectorAll<HTMLElement>(
      ".search-results .result-item, .search-results .row, .result-list .result-item, .row"
    );
    rows.forEach((row) => {
      this.syncCoeButton(row);
      this.syncWikiButton(row);
      this.syncPoedbButton(row);
    });
  }
  ...
  private syncPoedbButton(row: HTMLElement) {
    ...
    const searchByButton = left.querySelector<HTMLButtonElement>("button.searchBy");
    const poedbUrl = this.getUniqueItemPoedbUrl(row);
    ...
    if (!button) {
      ...
      button.setAttribute("aria-label", "Open PoeDB");
  ...
  private getUniqueItemPoedbUrl(row: HTMLElement) {
    ...
    // Recover the English item name for the poedb slug. The header may be
    // English, bilingual "中文 (English)", or pure Chinese (translated).
    let english: string | undefined;
    const paren = raw.match(/\(([A-Za-z0-9][A-Za-z0-9 '’,.\-]*)\)\s*$/);
    if (paren) english = paren[1].trim();
    if (!english) {
      const rev = locale === "cn" ? cnNameReverse : twNameReverse;
      english = rev[raw] || rev[raw.split(" (")[0].trim()];
    }
    if (!english && /^[\x20-\x7E]+$/.test(raw)) english = raw;
    if (!english) return null;

    // "Foulborn" (穢生) uniques are variants of an existing unique and poedb
    // lists them on the BASE unique's page (e.g. /Headhunter, not
    // /Foulborn_Headhunter). Drop the Foulborn prefix so the link resolves.
    english = english.replace(/^Foulborn\s+/i, "");

    const slug = encodeURIComponent(
      english.replace(/['’]/g, "").replace(/\s+/g, "_")
    );
    return `https://poedb.tw/${locale}/${slug}`;
  }
